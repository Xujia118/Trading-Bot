...
class TechnicalAnalysis:
    ...
    def sell_consolidation(self):
        shifted_df = self.df.shift(1, fill_value=0)    

        # yesterday was a small candle
        small_candle = abs(shifted_df['Open'] - shifted_df['Close']) <= self.small_candle_tolerance * shifted_df['Open']

        # Flag
        flag = (shifted_df['High'] * (1 + self.consolidation_tolerance) >= np.maximum(self.df['Open'], self.df['Close'])) & \
                    (shifted_df['Low'] * (1 - self.consolidation_tolerance) <= np.minimum(self.df['Open'], self.df['Close']))
        
        self.df['sell_consolidation'] = small_candle & flag                                          
    ...
    def buy_consolidation(self):
        ...
        # Bullish engulf
        bullish_engulf = (self.df['Close'] * (1 + self.consolidation_tolerance) >= np.maximum(shifted_df['Open'], shifted_df['Close'])) & \
                        (self.df['Open'] * (1 - self.consolidation_tolerance) <= np.minimum(shifted_df['Open'], shifted_df['Close']))
        
        # Flag
        flag = (shifted_df['High'] * (1 + self.consolidation_tolerance) >= np.maximum(self.df['Open'], self.df['Close'])) & \
                    (shifted_df['Low'] * (1 - self.consolidation_tolerance) <= np.minimum(self.df['Open'], self.df['Close']))

        self.df['buy_consolidation'] = (small_candle & bullish_engulf) | flag

        return self.df

    def good_to_buy(self, consolidation_days=5):  
        self.df = self.SMA60()  
        self.df = self.SMA120() 
    ...
    def good_to_sell(self, consolidation_days=3):
        self.df = self.Bollinger()
